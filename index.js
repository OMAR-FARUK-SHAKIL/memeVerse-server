...
client.connect(err => {
    console.log(err);
    const userCollection = client.db(`${process.env.DATABASE}`).collection("userCollection");
    const blogCollection = client.db(`${process.env.DATABASE}`).collection("blogCollection");
    const commentCollection = client.db(`${process.env.DATABASE}`).collection("commentCollection");
    const questionCollection = client.db(`${process.env.DATABASE}`).collection("questionCollection");
    const answerCollection = client.db(`${process.env.DATABASE}`).collection("answerCollection");
